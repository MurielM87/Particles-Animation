...
class Tripole {
    ...
    calculateFitness() {
        const nearbyTripoles = this.nearby(250); // Check within a radius of 50 for simplicity.
    ...
    render(ctx, parentRadius = null, parentPosition) {
        ...
        ctx.beginPath();
        ctx.arc(pos.x * __scale/2, pos.y * __scale/2, Math.sqrt(radius*__scale), 0, Math.PI * 2);
    ...
    mediate_entropy(sensor_data) {
        ...
        if (sensor_data.energy > 0) {
            let entropyRotation = this.calculate_entropy_rotation(sensor_data.entropy);
            this._entropic_degree = (this.entropic_degree + entropyRotation) % 360;
        }
    ...
    absorb_entropy(from, energy, entropy) {
        ...
        if (isTouching && isInside) {
            ...
            while (isTouching) {
                ...
                this.particleSystem.emit(
                    this.pos,
                    from.pos.sub(this.pos).norm().mul(1/distance*distance),  // Particle velocity coming from the other tripole
                    10000,
                    'rgba(0, 255, 0, 1)' 
                );
            }
        ...
        const newPos = this.pos.add(from.pos.sub(this.pos).norm().mul(1 / distance));
        if (newPos.x == newPos.x && newPos.y == newPos.y) {
            this.pos = this.pos.add(from.pos.sub(this.pos).norm().mul(1 / distance * 16));
        }
        // Emitting particles representing the transfer
        this.particleSystem.emit(
            this.pos,
            from.pos.sub(this.pos).norm().mul(-1/distance*distance*distance),  // Particle velocity coming from the other tripole
            6000,  // Lifetime of the particles
            'rgba(255, 0, 255, 1)'  // Bluish color for entropy particle
        );
    ...
    emit_entropy(to, energy, entropy) {
        ...
        this.particleSystem.emit(
            this.pos,
            to.pos.sub(this.pos).norm().mul(1 / distance * distance),  // Particle velocity coming from the other tripole
            6000,  // Lifetime of the particles
            'rgba(191, 0, 0, 1)'  // Bluish color for entropy particle
        );
        return energy * scale;
    ...
    absorbPotentialChildren() {
        for (let i = this.objects.length - 1; i >= 0; i--) {
            ...
            if (this.energy > child.energy * 1.5) {
                this.absorb(child);
            }
        }
    }
    ...
    absorb(child) {
        this.data.energy += child.energy;
        this.data.entropy += child.entropy;
    ...
    color() {
        ...
        const hue = (1 - entropyRatio * 2) * 360; // Inverse so higher entropy is cooler (toward blue)
        let lightness = 110 + (energyRatio - 0.9) * 20; // Energy affects lightness
...
class Particle {
    ...
    render(ctx) {
        ...
        ctx.beginPath();
        ctx.arc(this.pos.x * __scale, this.pos.y * __scale, 1, 0, Math.PI * 2);
        ctx.fillStyle = this.color;
        ctx.fill();
    }
}
...
function createTripole() {
    let tripole = new Tripole('bob', environment, new Vector(Math.random() * canvas.width, Math.random() * canvas.height));
    tripole.data.energy = Math.random() * 5
    tripole.data.entropy = .01
    tripole.sensor = new Tripole('sensor', tripole, tripole.pos);
    tripole.sensor.data.energy = Math.random() * 5
    tripole.sensor.data.entropy = .01
    tripole.actuator = new Tripole('actuator', tripole, tripole.pos);
    tripole.actuator.data.energy = Math.random() * 5
...
const whiteHole = new WhiteHole(environment, new Vector(100, canvas.height / 2), 5);
const blackHole = new BlackHole(environment, new Vector(canvas.width - 100, canvas.height / 2), 5);
...
function tripoleInteraction(tripoleA, tripoleB) {
    ...
    if (isTouching && isInside) {
        ...
        let correction = direction.mul(overlap / 4);
        ...
        correction = direction.mul(overlap / 4); // Use overlap to push apart by half the overlap distance for each
        tripoleA.pos = tripoleA.pos.add(correction);
        tripoleB.pos = tripoleB.pos.sub(correction);
      
         tripoleA.particleSystem.emit(
          tripoleA.pos,
          tripoleB.pos.sub(tripoleA.pos).norm().mul(-2*tripoleA.radius),  // Particle velocity towards the other tripole
          6000,  // Lifetime of the particles
          'rgba(0, 0, 215, 1)'  // Yellowish color for energy particle
        );
        tripoleB.particleSystem.emit(
          tripoleB.pos,
          tripoleA.pos.sub(tripoleB.pos).norm().mul(2*tripoleB.radius),  // Particle velocity towards the other tripole
          6000,  // Lifetime of the particles
          'rgba(215, 0, 0, 1)'  // Yellowish color for energy particle
        );
        ...
        if (_from.energy > 0 ) _from.emit_entropy(_to, 1.0001/distance**2, 1.0001/distance**2 * _from.energy / _from.entropy);

    } else if (distance < 100) {
        // Emit energy from the larger tripoles to the smaller one
        const _from = (tripoleA.energy / tripoleA.entropy) < (tripoleB.energy / tripoleB.entropy) ? tripoleA : tripoleB;
        const _to = (tripoleA.energy / tripoleA.entropy) < (tripoleB.energy / tripoleB.entropy) ? tripoleB : tripoleA;
        if (_from.energy > 0) _to.absorb_entropy(_from, 1.0001/distance**2, 1.0001/distance**2 * _from.energy / _from.entropy);
    }  else if (distance > 100) {
        // Emit energy from the larger tripoles to the smaller one
        const _from = (tripoleA.energy / tripoleA.entropy) < (tripoleB.energy / tripoleB.entropy) ? tripoleA : tripoleB;
        const _to = (tripoleA.energy / tripoleA.entropy) < (tripoleB.energy / tripoleB.entropy) ? tripoleB : tripoleA;
        if (_from.energy > 0) {
            _from.emit_entropy(_to, 1/distance**2, 1/distance**2 * _from.energy / _from.entropy);
        } else {
          _to.emit_entropy(_from, 1/distance**2, 1/distance**2 * _from.energy / _from.entropy);
        }
        ...
        if (_from.energy > 0) {
            ...
            _from.emit_entropy(_to, .01/distance**2, .01/distance**2 * _from.energy / _from.entropy);
        } else {
          _to.emit_entropy(_from, .01/distance**2, .01/distance**2 * _from.energy / _from.entropy);
        }
    } else {
        // Emit energy from the larger tripoles to the smaller one
        const _from = (tripoleA.energy) < (tripoleB.energy) ? tripoleA : tripoleB;
        const _to = (tripoleA.energy) < (tripoleB.energy) ? tripoleB : tripoleA;
        if (_from.energy > 0) {
            _from.emit_entropy(_to, _from.energy/distance, _from.energy/distance);
        } else {
          _to.emit_entropy(_from, _to.energy/distance, _to.energy/distance);
        }
    }
}
...
canvas.style.backgroundColor = '#000000ff';
render();
